...
Due to the nature of this COMSOL optimization workflow, which requires reimporting the mesh within the same model, generating a compact history or a concise .m file to describe the workflow is challenging. If you're interested in accessing the .mph file, please feel free to contact either me o [my supervisor](https://samurai.nims.go.jp/profiles/yoshikawa_genki?locale=en).

In this optimization process, the internal slots for parametric sweeps are already utilized for parameter continuation and robust formulation. This makes it difficult to sweep over different optimization conditions. To address this issue, we use MATLAB LiveLink to automate the parameter sweep. The relevant code is available in this [Github repository](https://github.com/Chaozhuang22/COMSOL-MATLAB-livelink).
